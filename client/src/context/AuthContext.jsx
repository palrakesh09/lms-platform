...
export { AUTH_STATUS } from './AuthContext.js';
...
export function AuthProvider({ children }) {
  ...
  // Called when an API request answers 401 while the UI still thinks the user is logged in
  // (the session expired or the account was disabled). Route guards then redirect to login.
  // No API call: the server already rejected the session, and its 401 response cleared the cookie.
  const expireSession = useCallback(() => setState(GUEST_STATE), []);
  ...
  const value = useMemo(
    ...
    [state, login, register, logout, expireSession],
  );
